...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        "Ох, приветствую тебя, Андрей, Завоеватель Трех с\n"
        "Половиной Десятилетий! 🥳\n"
        "Я, Магистр Флавиус Абессинский, Архимаг Школы Трансмутации и\n"
        "Небесный Предсказатель Скидок, прозреваю сквозь туманы измерений, чтобы узреть\n"
        "твое великое... <b>повышение уровня!</b> 🌟\n"
        "Желаю <b>Критических Успехов</b> во всех делах, бесконечного запаса\n"
        "<b>Хит Поинтов</b> для здоровья и золота, чтобы всегда хватало на лучшие зелья и\n"
        "артефакты. 💎 Пусть каждый твой день дарит <b>Легендарный лут!</b> С 35-м уровнем! 🎂\n\n"
        "Твоя доблестная душа и заслуженный возраст притягивают не только драконов, но и сокровища! 🐲 Я, Магистр Флавиус, сообщаю: твои подарки не достанутся"
        "тебе легко. Они запечатаны <b>Мощным Заклинанием Временного Смещения</b>, которое можно снять только выполнив <b>Квест Дня Рождения!</b> 📜\n"
        "Собери свой отряд, заточи <b>Меч Острых Шуток</b> и отправляйся! И\n"
        "помни: каждый выполненный этап Квеста дает не только <b>Опыт</b>, но и <b>Часть Награды!</b> 💰\n"
...
async def start_quest_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        "Андрей, ты принял вызов! Мудро! 🧠 Но для начала, ты должен\n"
        "доказать, что глаз твой остер, а память о <b>Твоем Личном Подземелье</b> — безупречна. ✨\n\n"
        "<b>ЗАДАНИЕ 1: ИСПЫТАНИЕ НАБЛЮДАТЕЛЬНОСТИ</b> 🧐\n"
        "Я использовал свое могущественное <b>Заклинание Иллюзии 4-го\n"
        "Круга</b> и слегка изменил одну фотографию, на которой изображен угол твоего\n"
        "мирного убежища. Это уголок, который ты видишь постоянно, почти на уровне\n"
        "подсознания. 👁️\n\n"
        "Твоя задача, Герой: Взгляни на присланный образ. <b>Что не так?</b>\n"
        "<b>Какой Важный Артефакт</b>, который должен быть на этом месте, был временно изгнан\n"
        "из этого Измерения моей магией? 🔮\n\n"
        "Удачи, Искатель Сокровищ! Да будет твоя проницательность так\n"
        "же остра, как когти! 🦅"
    )
...
async def task_1_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer.strip().lower() == "криминальное чтиво":
        text = (
            "<b>БРАВО, АНДРЕЙ!</b> Твой <b>Бросок на Интеллект</b> успешен,\n"
            "<b>Критический Успех!</b> 🎯 Ты прорвался сквозь мои чары! Задание выполнено! ✅\n\n"
            "Но не расслабляйся! Твое следующее испытание требует\n"
            "<b>Кошачьей Перспективы!</b> 🐱\n\n"
            "<b>ЗАДАНИЕ 2: ТОЧКА ЗРЕНИЯ АРТЕФАКТА</b> 👁️\n"
            "Я использовал <b>Заклинание Смещения Души</b> и временно поместил\n"
            "один из твоих подарков в... необычное место. 🎁\n\n"
            "Тебе будет отправлена фотография. Но это не просто фото! Это\n"
            "ракурс, который видит только твой подарок. <b>Ты смотришь на мир его глазами!</b>\n\n"
            "Твоя задача: <b>Найти место в твоем Логове</b>, где сейчас\n"
            "находится этот тайный Артефакт! 🗺️\n\n"
            "Ориентируйся по потолку, полу, мебели! Думай, как <b>Спящий\n"
            "Кот</b>, который наблюдает за миром из своего укромного места! 💤\n\n"
...
async def task_2_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer.strip().lower() == "марсель":
        text = (
            "<b>БЛЕСТЯЩЕ, АНДРЕЙ!</b> Ты обладаешь <b>Повышенным Интеллектом</b>\n"
            "замлуженно! 💡 Ты верно отгадал шараду! Задание 2 выполнено! ✅\n\n"
            "Но следующая награда требует ещё большего усилия! 💪\n\n"
            "<b>ЗАДАНИЕ 3: ТАКТИЛЬНОЕ ИСПЫТАНИЕ</b> 🖐️\n"
            "Я использовал свое <b>Заклинание Трансмутации Поверхности</b> и\n"
            "слил твой последний спрятанный дар с окружающей средой! 🌿\n\n"
            "Тебе будет отправлена фотография <b>крупного плана уникальной\n"
            "текстуры</b> в твоем Логове. 🖼️\n\n"
            "Твоя задача: <b>Найти этот узор в твоей квартире!</b> Последний\n"
            "Подарок спрятан именно <b>В ЭТОМ МЕСТЕ</b>, слившись с ним! 🤫\n\n"
            "Сконцентрируй свою <b>Магию Осязания!</b> Думай, как <b>Кот</b>, который\n"
            "выбирает самое мягкое и незаметное место для сна! 😴\n\n"
...
async def task_3_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer.strip() == "7":
        text = (
            ...
            "Но теперь ты стоишь перед <b>Сундуком Сокровищ!</b> 📦 Он заперт\n"
            "древним <b>Замком Чисел</b>, который откроет только верный код! 🔐\n\n"
            "<b>ФИНАЛЬНОЕ ИСПЫТАНИЕ: ЛАБИРИНТ ХРАНИТЕЛЯ КОДА</b> 🗺️\n"
            "Перед тобой лежит Лабиринт, начертанный на листе (или\n"
            "экране). Это <b>Проклятый Лабиринт Флавиуса</b>, где все пути ведут к истине.\n\n"
            ...
            "Сконцентрируй свою <b>Ловкость и Интеллект!</b> Только верный код\n"
            "отопрет Сундук, где тебя ждет <b>Легендарный Лут Героя 35-го Уровня!</b> 🏆\n\n"
...
async def task_4_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer.strip() == "99":
        text = (
            ...
            "Твоя цель — <b>Главный Артефакт</b> — находится <b>РЯДОМ!</b> Так близко,\n"
            "как никогда! 🤩\n\n"
            "Я присылаю тебе <b>Трио Предвестников!</b> Ты знаешь <b>Точки Респауна</b>\n"
            "этих предметов? 🤔\n\n"
            "Твоя задача, Герой: <b>Отправляйся в Зону Наибольшего\n"
            "Сближения!</b> Найди то место, где <b>ВСЕ ТРИ ПРЕДМЕТА</b> сходятся на <b>Карте Твоего Дома!</b> 🗺️\n\n"
            "Именно там, в <b>Центре Притяжения</b> этих Артефактов, лежит ТВОЙ\n"
            "<b>ФИНАЛЬНЫЙ, СИЯЮЩИЙ, ЛЕГЕНДАРНЫЙ ПОДАРОК!</b> 🎁✨\n\n"
...
async def task_5_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        "Что ты тут ищешь, Андрей? Твой <b>Журнал Квестов</b> должен быть\n"
        "пуст, а <b>Инвентарь</b> — полон! 📜 Ты завершил все Испытания? Нашел <b>Финальный Артефакт?</b> 🎁\n"
...
async def finish_quest_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        "<b>АНДРЕЙ, ТИТАН 35-ГО УРОВНЯ, ОФИЦИАЛЬНО ЗАВЕРШИЛ ЭПИЧЕСКУЮ\n"
        "КАМПАНИЮ ДНЯ РОЖДЕНИЯ!</b> 🏆\n\n"
        "Его <b>Интеллект</b> преодолел иллюзии, <b>Ловкость</b> обошла ловушки, а\n"
        "<b>Мудрость</b> привела к <b>Финальному Луту!</b> ✨ Ты доказал, что являешься <b>Истинным\n"
        "Властелином своего Логова!</b> 👑 Твой <b>Счет Опыта</b> сегодня достиг легендарных высот! 📈\n\n"
        "Пусть твой <b>Модификатор Харизмы</b> очаровывает всех союзников, 🥰\n"
        "твой <b>Спасбросок от Усталости</b> всегда будет успешен, а твоя <b>Куча Золота</b> никогда\n"
        "не уменьшается! 💰\n\n"
        "<b>МЫ САЛЮТУЕМ ТЕБЕ, АНДРЕЙ!</b> Отпразднуй свою Победу с честью и\n"
        "изобилием! 🥳\n"
        "<b>С ДНEM РОЖДЕНИЯ!</b> 🎂"
    )
...
async def after_finish_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        "Как только <b>Финальный Лут</b> был добыт, моя <b>Магическая\n"
        "Резерва</b> опустела. 📉 Я активировал <b>Заклинание\n"
        "Экстренного Отступления</b> (Emergency Retreat) и сейчас восстанавливаю <b>Очки Маны</b> в <b>Астральном\n"
        "Трактире \"Спящий Кот\"!</b> 😴\n\n"
        "Мой <b>Договор Исполнен!</b> 📜 Твой <b>Квест Завершен!</b> Ты свободен! Я\n"
        "вернусь, когда соберу достаточно <b>Реагентов</b> для следующего <b>Эпического\n"
        "Приключения!</b> 🧙‍♂️\n\n"
